    ...
    public class WebCamTextureToMatExample : MonoBehaviour
    {
        ...
        void Update()
        {
            if (hasInitDone && webCamTexture.isPlaying && webCamTexture.didUpdateThisFrame)
            {
                ...
                Imgproc.dilate(rgbaMat, rgbaMat, Kernel, new Point(-1, -1), 10);
                Imgproc.morphologyEx(rgbaMat, rgbaMat, Imgproc.MORPH_CLOSE, Kernel, new Point(-1, -1), 10);
                
                Imgproc.cvtColor(rgbaMat, hsv, Imgproc.COLOR_BGR2HSV);
                Core.inRange(hsv, new Scalar(0, 100, 100), new Scalar(50, 255, 255), hsv);
                Core.bitwise_and(rgbaMat, rgbaMat, dst, hsv);
                
                Mat grayscale = new Mat();
                Mat circles = new Mat();
                Imgproc.cvtColor(dst, grayscale, Imgproc.COLOR_BGR2GRAY);
                Imgproc.HoughCircles(grayscale, circles, Imgproc.CV_HOUGH_GRADIENT, 1, 20, 50, 30, 0, 0);
                Point pt = new Point();
                ...
                for(int i=0; i<circles.cols(); i++){
                    ...
                    pt.y = data[1];
                    double rho = data[2];
                    Imgproc.circle(dst, pt, (int)rho, new Scalar(255, 0, 0, 255), 5);
                }
                ...
                */
                ...
                Imgproc.GaussianBlur(rgbaMat, dst, new Size(5, 5), 1, 0, Core.BORDER_DEFAULT);
                Imgproc.Canny(dst, dst, 0, 100, 3, true);
                ...
                */
                ...
                Mat Kernel = Imgproc.getStructuringElement(Imgproc.MORPH_CROSS, new Size(3, 3));
                //Imgproc.dilate(rgbaMat, rgbaMat, Kernel, new Point(-1, -1), 10);
                //Imgproc.erode(rgbaMat, rgbaMat, Kernel, new Point(-1, -1), 10);
                // Imgproc.morphologyEx(rgbaMat, rgbaMat, Imgproc.MORPH_OPEN, Kernel, new Point(-1, -1), 10);
                Imgproc.morphologyEx(rgbaMat, rgbaMat, Imgproc.MORPH_CLOSE, Kernel, new Point(-1, -1), 10);
                ...
                /*2
                //Imgproc.bilateralFilter(rgbaMat, rgbaMat, 50, 3, 3, Core.BORDER_DEFAULT);
                Imgproc.cvtColor(rgbaMat, rgbaMat, Imgproc.COLOR_BGR2GRAY);
                Imgproc.GaussianBlur(rgbaMat, rgbaMat, new Size(255, 255), 0);
                Imgproc.threshold(rgbaMat, rgbaMat, 50, 255, Imgproc.THRESH_TRUNC);
                ...
                */
                ...
                /*1
                Mat dst = new Mat();
                Mat mat = Imgproc.getRotationMatrix2D(new Point(rgbaMat.width()/2, rgbaMat.height()/2), 90, 1);
                Imgproc.warpAffine(rgbaMat, dst, mat, new Size(rgbaMat.width(), rgbaMat.height()));
                Imgproc.cvtColor(rgbaMat, dst, Imgproc.COLOR_BGR2GRAY);
